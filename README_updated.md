...
```bash
flux bootstrap github   --owner=$GITHUB_USER   --repository=json-server-flux   --branch=main   --path=clusters/json-server   --personal   --components-extra=image-reflector-controller,image-automation-controller   --token-auth
```
...
## 11. Test the Running json-server
